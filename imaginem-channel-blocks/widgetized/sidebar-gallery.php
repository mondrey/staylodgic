...
class mtheme_SidebarGallery_Widget extends WP_Widget {
	function __construct() {
		...
		parent::__construct('mtheme_sidebar_gallery', __('superlens Gallery Widget'), $widget_ops);
	}
		...
		if ( !empty( $title ) ) { 
			...
			switch ($listchoice) {
				...
				case "portfolio" :
					...
					$thumbnail_image_id = get_post_thumbnail_id(($post->ID), 'superlens-gridblock-tiny'); 
					$thumbnail_image_url = wp_get_attachment_image_src($image_id,'superlens-gridblock-tiny');  
					...
					?>
						...
						if ($clickaction=="lightbox" && !post_password_required() ) { ?>
							<a class="lightbox-active lightbox-image" data-rel="gallery-sidebargallery" data-thumb="<?php echo esc_url($lightbox_url); ?>" data-src="<?php echo esc_url($lightbox_url); ?>" href="<?php echo esc_url($lightbox_url); ?>">
							<?php
							} else {
							?>
						...
						?>
								...
								echo superlens_display_post_image (
									$post->ID,
									$have_image_url,
									$link=false,
									$type="",
				...
				case "page" :
					if ( function_exists('superlens_get_custom_attachments') ) {
						$filter_image_ids = superlens_get_custom_attachments ( $thepage );
					}
					...
					if ( isSet($filter_image_ids) && isSet($thepage) ) {
						...
						$thumbnail_imagearray = wp_get_attachment_image_src( $attatchmentID , 'superlens-gridblock-tiny', false);
				...
				case "category" :
					...
					$thumbnail_image_id = get_post_thumbnail_id(($post->ID), 'superlens-gridblock-tiny'); 
					$thumbnail_image_url = wp_get_attachment_image_src($thumbnail_image_id,'superlens-gridblock-tiny'); 
					$thumbnail_image_url = $thumbnail_image_url[0];
