...
class Reservations {
	...
	public function __construct( $date = false, $room_id = false, $reservation_id = false, $reservation_id_excluded = false ) {
		...
		add_action( 'wp_ajax_get_available_rooms', array( $this, 'get_available_rooms' ) );
		add_action( 'wp_ajax_nopriv_get_available_rooms', array( $this, 'get_available_rooms' ) );

		add_action( 'wp_ajax_get_booking_details', array( $this, 'get_booking_details' ) );
		add_action( 'wp_ajax_nopriv_get_booking_details', array( $this, 'get_booking_details' ) );
	}
	...
	/**
	 * Method get_confirmed_reservations
	 *
	 * @return void
	 */
	...
	/**
	 * Method get_guest_for_reservation
	 *
	 * @param $booking_number
	 *
	 * @return void
	...
	public function get_reservations_for_room( $checkin_date = false, $checkout_date = false, $reservation_status = false, $reservation_substatus = false, $room_id = false ) {
		...
		if ( false !== $reservation_substatus ) {
			$meta_query[] = array(
		...
		if ( false !== $checkin_date || false !== $checkout_date ) {
			$meta_query[] = array(
				'relation' => 'AND',
				array(
	...
	/**
	 * Method get_room_reservations_for_date_range
	 *
	 ...
	 * @return void
	 */
	public function get_room_reservations_for_date_range( $stay_start_date, $stay_end_date, $the_room_id ) {
	...
	/**
	 * Method calculate_reserved_rooms
	 *
	 * @return void
	 */
	public function calculate_reserved_rooms() {
	...
	public function calculate_and_update_remaining_room_counts_for_all_dates( $room_id = false ) {
		...
		update_post_meta( $room_id, 'staylodgic_remaining_rooms_count', wp_json_encode( $remaining_rooms_count ) );
	 ...
	 */
	public function calculate_and_update_remaining_room_count( $date = false, $room_id = false ) {
	...
	public function get_remaining_room_count_array( $room_id = false ) {
		...
		$remaining_quantity_array_json = get_post_meta( $room_id, 'staylodgic_remaining_rooms_count', true );

		// Decode the JSON string
		$remaining_quantity_array = json_decode( $remaining_quantity_array_json, true );

		// Check if the result is an array, if not, return an empty array
		if ( ! is_array( $remaining_quantity_array ) ) {
			$remaining_quantity_array = array();
		}
	...
	public function get_direct_remaining_room_count( $date = false, $room_id = false ) {
		...
		if ( \Staylodgic\Rooms::is_channel_room_booked( $room_id, $date ) ) {
			return '0';
		}
	 ...
	 */
	public function get_total_remaining_for_all_rooms_on_date( $date = false ) {
		...
		foreach ( $rooms as $room ) {
			...
			$remaining_rooms_array = $this->get_remaining_room_count_array( $room->ID );

			// Add the remaining count for the specific date to the total
			if ( isset( $remaining_rooms_array[ $date ] ) ) {

				if ( \Staylodgic\Rooms::is_channel_room_booked( $room->ID, $date ) ) {
					$remaining_rooms_array[ $date ] = 0;
				}

				$total_remaining += $remaining_rooms_array[ $date ];
			}
		}
	...
	public function update_remaining_room_count( $room_id ) {
		...
		$stay_remaining_rooms_count = array();

		// Calculate remaining rooms for each date
		foreach ( $quantity_array as $date => $stay_total_rooms ) {
			$stay_reserved_rooms                 = isset( $reservations_array[ $date ] ) ? count( $reservations_array[ $date ] ) : 0;
			$stay_remaining_rooms_count[ $date ] = $stay_total_rooms - $stay_reserved_rooms;
		}

		// Update the remaining rooms count meta field
		update_post_meta( $room_id, 'staylodgic_remaining_rooms_count', wp_json_encode( $stay_remaining_rooms_count ) );
	}

	/**
	 * Method count_reservations_for_day
	 *
	 * @param $room_id $room_id [explicite description]
	 ...
	 * @return void
	 */
	public function count_reservations_for_day( $room_id = false, $day = false, $excluded_reservation_id = false ) {
		...
		if ( isset( $reservations_array[ $day ] ) ) {
			...
			if ( is_array( $reservation_ids ) ) {
				...
				foreach ( $reservation_ids as $reservation_id ) {
					...
					if ( (int) $reservation_id === (int) $excluded_reservation_id ) {
						continue;
					}
				...
				$max_room_count = \Staylodgic\Rooms::get_max_quantity_for_room( $room_id, $day );
	...
	public function get_reservation_guest_name( $booking_number = false ) {
		...
		$customer_query = $this->get_guest_for_reservation( $booking_number );
	...
	/**
	 * Method is_guest_currently_staying
	 *
	 * @return void
	 */
	public function is_guest_currently_staying() {
	...
	/**
	 * Method is_guest_checking_in_today
	 *
	 * @return void
	 */
	public function is_guest_checking_in_today() {
		$reservation_post_id = $this->reservation_id;
		$today_date          = gmdate( 'Y-m-d' ); // Get today's date
	...
	/**
	 * Method is_guest_checking_out_today
	 *
	 * @return void
	 */
	public function is_guest_checking_out_today() {
		$reservation_post_id = $this->reservation_id;
		$today_date          = gmdate( 'Y-m-d' ); // Get today's date
	...
	/**
	 * Method get_room_ids_for_booking_number
	 *
	 * @param $booking_number $booking_number [explicite description]
	 *
	 * @return void
	 */
	public static function get_room_ids_for_booking_number( $booking_number ) {
	...
	/**
	 * Method get_room_title_for_reservation
	 *
	 * @param $reservation_id $reservation_id [explicite description]
	 *
	 * @return void
	 */
	public function get_room_title_for_reservation( $reservation_id = false ) {
	...
	/**
	 * Summary of get_reservation_ids_for_customer
	...
	/**
	 * Method get_edit_links_for_reservations
	 *
	 * @param $reservation_array $reservation_array [explicite description]
	 *
	 * @return void
	 */
	public function get_edit_links_for_reservations( $reservation_array ) {
		$links = '<ul>';
	...
	public function is_room_for_the_day_fullybooked( $stay_room_id = false, $stay_date_string = false, $excluded_reservation_id = null ) {
		...
		$reserved_room_count = $this->count_reservations_for_day( $room_id, $day = $stay_date_string, $excluded_reservation_id );
	...
	/**
	 * Method split_array_by_continuous_days
	 *
	 * @param $input_array $input_array [explicite description]
	 *
	 * @return void
	 */
	public function split_array_by_continuous_days( $input_array ) {
		$output_array = array();

		foreach ( $input_array as $key => $dates ) {
			$temp_array    = array();
			$subset        = array();
			$previous_date = null;

			foreach ( $dates as $date => $value ) {
				if ( null === $previous_date || ( strtotime( $previous_date . ' + 1 day' ) === strtotime( $date ) ) ) {
					$subset[ $date ] = $value;
				} else {
					$temp_array[] = $subset;
	...
	public function days_fully_booked_for_date_range( $checkin_date = false, $checkout_date = false ) {
		...
		foreach ( $daterange as $date ) {
			...
			$daily_room_availability[ $date_string ] = 0;
		}
		...
		foreach ( $room_list as $room ) {
			foreach ( $daterange as $date ) {
				...
				$daily_room_availability[ $date_string ] += $remaining_rooms;
			}
		}

		// Identify fully booked days
		$fully_booked_days = array();
		foreach ( $daily_room_availability as $date => $availability ) {
			if ( 0 === $availability ) {
				$fully_booked_days[] = $date;
			}
		}
	...
	public function availability_of_rooms_for_date_range( $checkin_date = false, $checkout_date = false, $limit = 10 ) {
		...
		foreach ( $room_list as $room ) {
			foreach ( $daterange as $date ) {
				...
				$reservationid = false;
				if ( ! $this->is_room_for_the_day_fullybooked( $room->ID, $date_string, $reservationid ) ) {
		...
		$sub_set_room_availablity = self::split_array_by_continuous_days( $room_availablity );

		return $sub_set_room_availablity;
	}

	/**
	 * Method is_room_fullybooked_for_date_range
	 *
	 * @param $stay_room_id $stay_room_id [explicite description]
	 * @param $checkin_date $checkin_date [explicite description]
	 * @param $checkout_date $checkout_date [explicite description]
	 ...
	 * @return void
	 */
	public function is_room_fullybooked_for_date_range( $stay_room_id = false, $checkin_date = false, $checkout_date = false, $reservationid = false ) {
	...
	public function was_room_ever_opened( $stay_date_string = false, $room_id = false ) {
		...
		$max_count = \Staylodgic\Rooms::get_max_quantity_for_room( $room_id, $stay_date_string );
		return $max_count;
	}
	...
	public function remaining_rooms_for_day( $stay_date_string = false, $room_id = false, $excluded_reservation_id = false ) {
		...
		$reserved_room_count = $this->count_reservations_for_day( $room_id, $stay_date_string, $excluded_reservation_id );
	...
	public function build_reservations_data_for_room_for_day( $reservations, $reservation_status = false, $reservation_substatus = false, $stay_date_string = false, $room_id = false ) {
		...
		if ( $reservations->have_posts() ) {
			...
			while ( $reservations->have_posts() ) {
				...
				$stay_reserved_rooms[] = get_the_ID();
				$reservation_id        = get_the_ID();
				$custom                = get_post_custom( get_the_ID() );
				if ( isset( $custom['staylodgic_reservation_checkin'][0] ) ) {
					$date_range_value = $custom['staylodgic_reservation_checkin'][0];
				}
				...
				$stay_number_of_days    = floor( ( $reservation_end_date - $reservation_start_date ) / ( 60 * 60 * 24 ) ) + 1;

				if ( (int) $post_room_id === (int) $room_id ) {
					...
					if ( $stay_current_date >= $reservation_start_date && $stay_current_date < $reservation_end_date ) {
						...
						$reservation_duration = floor( ( $reservation_end_date - $reservation_start_date ) / ( 60 * 60 * 24 ) ) + 1;
						if ( $stay_number_of_days > 0 ) {
	...
	public function get_available_rooms() {
		...
		foreach ( $room_list as $room ) {
			$is_fullybooked = $this->is_room_fullybooked_for_date_range( $room->ID, $checkin_date, $checkout_date, $reservation_id = $reservationid );
		...
		echo wp_json_encode( $available_rooms );
