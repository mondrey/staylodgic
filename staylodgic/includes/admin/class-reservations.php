...
class Reservations {
	...
	public function update_remaining_room_count( $room_id ) {
		...
		$got_remaining_rooms_count = array();

		// Calculate remaining rooms for each date
		foreach ( $quantity_array as $date => $got_total_rooms ) {
			$got_reserved_rooms                 = isset( $reservations_array[ $date ] ) ? count( $reservations_array[ $date ] ) : 0;
			$got_remaining_rooms_count[ $date ] = $got_total_rooms - $got_reserved_rooms;
		}

		// Update the remaining rooms count meta field
		update_post_meta( $room_id, 'staylodgic_remaining_rooms_count', wp_json_encode( $got_remaining_rooms_count ) );
	}
	...
	/**
	 * Method is_guest_checkingin_today
	 *
	 * @return void
	 */
	public function is_guest_checkingin_today() {
		$reservation_post_id = $this->reservation_id;
		$today_date          = gmdate( 'Y-m-d' ); // Get today's date
	...
	/**
	 * Method is_guest_checkingout_today
	 *
	 * @return void
	 */
	public function is_guest_checkingout_today() {
		$reservation_post_id = $this->reservation_id;
		$today_date          = gmdate( 'Y-m-d' ); // Get today's date
	...
	public function is_room_for_the_day_fullybooked( $stay_room_id = false, $stay_date_string = false, $excluded_reservation_id = null ) {
		...
		$reserved_room_count = $this->count_reservations_for_day( $room_id = $stay_room_id, $day = $stay_date_string, $excluded_reservation_id );
